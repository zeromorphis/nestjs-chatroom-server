...
export class ChatGateway {
  constructor(
    ...
    this.defaultGroupId = DEFAULT_GROUP.groupId;
  }
  ...
  async handleConnection(client: Socket): Promise<string> {
    ...
    // 连接默认加入"默认的聊天室"房间
    client.join(this.defaultGroupId);
    // 进来统计一下在线人数
  ...
  async editGroupName(@ConnectedSocket() client: Socket, @MessageBody() data: Group): Promise<any> {
    const isUser = await this.userRepository.findOne({ userId: data.userId });
    if (isUser) {
      const isHaveGroup = await this.groupRepository.findOne({ groupName: data.groupName });
      if (isHaveGroup) {
        return this.server.to(data.userId).emit('editGroupName', { code: RCode.FAIL, msg: '该群名已存在' });
      } else {
        const group = await this.groupRepository.findOne({ groupId: data.groupId });
        if (data.groupId === this.defaultGroupId) {
          return this.server.to(data.userId).emit('editGroupName', { code: RCode.FAIL, msg: '默认群不修改' });
        } else if (data.userId === group.userId) {
          const newGroup = JSON.parse(JSON.stringify(group));
          newGroup.groupName = data.groupName;
          await this.groupRepository.update(group, newGroup);
          return this.server.to(data.userId).emit('editGroupName', { code: RCode.OK, msg: '群名称修改成功', data: newGroup });
        } else {
          return this.server.to(data.userId).emit('editGroupName', { code: RCode.FAIL, msg: `无权限修改群名称` });
        }
      }
    } else {
      return this.server.to(data.userId).emit('editGroup', { code: RCode.FAIL, msg: `你没资格修改群名称` });
    }
  }

  // 编辑群公告
  @SubscribeMessage('editGroupNotice')
  async editGroupNotice(@ConnectedSocket() client: Socket, @MessageBody() data: Group): Promise<any> {
    const isUser = await this.userRepository.findOne({ userId: data.userId });
    if (isUser) {
      const group = await this.groupRepository.findOne({ groupId: data.groupId });
      if (data.notice === group.notice) {
        return this.server.to(data.userId).emit('editGroupNotice', { code: RCode.FAIL, msg: `请发布新的群公告` });
      } else if ((data.userId === group.userId) || (data.groupId === this.defaultGroupId)) {
        const newGroup = JSON.parse(JSON.stringify(group));
        newGroup.notice = data.notice;
        await this.groupRepository.update(group, newGroup);
        return this.server.to(data.userId).emit('editGroupNotice', { code: RCode.OK, msg: '群公告修改成功', data: newGroup });
      } else {
        return this.server.to(data.userId).emit('editGroupNotice', { code: RCode.FAIL, msg: `无权限修改群公告` });
      }
    } else {
      return this.server.to(data.userId).emit('editGroupNotice', { code: RCode.FAIL, msg: `你没资格修改群公告` });
    }
  }
  ...
  async exitGroup(@ConnectedSocket() client: Socket, @MessageBody() groupMap: GroupMap): Promise<any> {
    if (groupMap.groupId === this.defaultGroupId) {
  ...
  async getActiveGroupUser() {
    ...
    this.server.to(this.defaultGroupId).emit('activeGroupUser', {
      msg: 'activeGroupUser',
